...
def usage():
    ...
    - dose_matrix.csv is the file containing the dosing information for each drug (real concentations corresponding to coded concentrations)
...
def dilution_factors(dose_matrix, stock, mixing_matrix, min_num_doses, max_num_doses):
    # Initialize
    num_doses = max_num_doses + 1 - min_num_doses
    num_drugs = len(mixing_matrix.index)
    dilution_matrix = pd.DataFrame(
        index=mixing_matrix.index, #rows, i.e. drugs
        columns=range(min_num_doses, max_num_doses + 1), #columns, i.e. number of doses
        data=np.zeros((num_drugs, num_doses), dtype=int))


    return dilution_matrix
...
def main():
    ...
    dilution_matrix = dilution_factors(dose_matrix, stock, mixing_matrix, min_num_doses, max_num_doses)
